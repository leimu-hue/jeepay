...
ALTER TABLE `t_mch_notify_record`
    ADD COLUMN `notify_count_limit` INT(11) NOT NULL DEFAULT '6' COMMENT '最大通知次数, 默认6次' after `notify_count`;
## -- ++++ ++++
...
ALTER TABLE `t_pay_interface_define`
    ADD COLUMN `config_page_type` TINYINT(6) NOT NULL DEFAULT 1 COMMENT '支付参数配置页面类型:1-JSON渲染,2-自定义' after `is_isv_mode`;

-- 优化支付接口定义初始化，新增是否为脱敏数据
DELETE
FROM t_pay_interface_define
WHERE if_code = 'alipay';
...
alter table `t_pay_order`
    add column `mch_fee_rate` decimal(20, 6) NOT NULL COMMENT '商户手续费费率快照' after `amount`;
alter table `t_pay_order`
    add column `mch_fee_amount` BIGINT(20) NOT NULL COMMENT '商户手续费,单位分' after `mch_fee_rate`;
update `t_pay_order`
set mch_fee_rate = 0;
update `t_pay_order`
set mch_fee_amount = 0;

alter table `t_pay_order` drop column `division_flag`;
alter table `t_pay_order` drop column `division_time`;

alter table `t_pay_order`
    add column `division_mode` TINYINT(6) DEFAULT 0 COMMENT '订单分账模式：0-该笔订单不允许分账, 1-支付成功按配置自动完成分账, 2-商户手动分账(解冻商户金额)' after `refund_amount`;
alter table `t_pay_order`
    add column `division_state` TINYINT(6) DEFAULT 0 COMMENT '订单分账状态：0-未发生分账, 1-等待分账任务处理, 2-分账处理中, 3-分账任务已结束(不体现状态)' after `division_mode`;
alter table `t_pay_order`
    add column `division_last_time` DATETIME COMMENT '最新分账时间' after `division_state`;
...
alter table t_transfer_order
    add column `channel_res_data` TEXT DEFAULT NULL COMMENT '渠道响应数据（如微信确认数据包）' after `channel_order_no`;
